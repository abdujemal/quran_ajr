...
import 'package:quran_ajr/search.dart';
...
class _QuranPageState extends State<QuranPage> {
  ...
  initilize() async {
    ...
    if (widget.sura[0] == 1) {
      ayat = await db
          .rawQuery("SELECT * FROM 'quran' WHERE sura='${widget.sura[0]}'");
    } else {
      ayat.addAll(await db
          .rawQuery("SELECT * FROM 'quran' WHERE sura='${widget.sura[0]}'"));
    }
        ...
        child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              return GestureDetector(
                onLongPress: ayat[index]["text"] !=
                        "بِسْمِ ٱللَّهِ ٱلرَّحْمَٰنِ ٱلرَّحِيمِ"
                    ? () {
                        showDialog(
                            context: context,
                            builder: (context) =>
                                CheckDialog(aya: ayat[index]));
                      }
                    : () {},
                child: Container(
                  child: Padding(
                    ...
                    child: Container(
                      ...
                      child: Padding(
                        ...
                        child: Text(
                          ayat[index]["text"],
                          textAlign: ayat[index]["text"] !=
                                  "بِسْمِ ٱللَّهِ ٱلرَّحْمَٰنِ ٱلرَّحِيمِ"
                              ? TextAlign.start
                              : TextAlign.center,
                          textDirection: TextDirection.rtl,
                          style: ayat[index]["text"] !=
                                  "بِسْمِ ٱللَّهِ ٱلرَّحْمَٰنِ ٱلرَّحِيمِ"
